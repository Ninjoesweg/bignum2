...
class LList implements List {
    ...
    //checks whether the Linked list contains an item returning true or false depending on result
    public boolean contains(Object item){
        boolean value=false;
        //start at the head move through as long as the current pos isn't greater than listsize then move current pos to next node
        for (moveToStart();currPos()<length(); next()){
            //if current position is == to value of item
            value=getValue() == item;
        }
        return value;
    }
    //finds the value of a item in the Linked list and outputs it
    public Object get(int index){
        Object value = new LList();//new object to hold value to be returned
        for(moveToStart();currPos()<length();next()){
            //check that current element is equal to the value of  the derived value of the index from the get funct
            if (currPos()==index){
                value=getValue();//gives the value of the index that passes my test
            }

        }

        return value; //returning the value that I want
    }
}
